...
export const mockIoTDataCron = schedules.task({
  ...
  run: async (payload) => {
    ...
    const sensorTypes = [
      { type: 'temperature', unit: '°C', min: 0, max: 50 },
      { type: 'ph', unit: '', min: 0, max: 14 },
      { type: 'dissolvedOxygen', unit: 'mg/L', min: 0, max: 20 },
      { type: 'turbidity', unit: 'NTU', min: 0, max: 200 },
      {
        type: 'conductivity',
        unit: 'µS/cm',
        min: 0,
      ...
      { type: 'flowRate', unit: 'm³/h', min: 0, max: 200 },
      { type: 'tds', unit: 'ppm', min: 0, max: 5000 },
    ];
